...
#   ./rotate-secrets.sh rotate-cursor-forward [new-token-hex]
...
cmd_rotate_cursor_forward() {
  require_az
  local token="${1:-$(require_openssl && gen_token)}"
  echo "WARNING: rotate UrNammu FIRST to avoid a 401 window."
  echo "  New cursor forward token (copy this to Vercel env before continuing):"
  echo "    $token"
  echo
  echo "  1. vercel env rm CURSOR_TELEMETRY_SECRET production -y"
  echo "  2. echo \"$token\" | vercel env add CURSOR_TELEMETRY_SECRET production"
  echo "  3. vercel --prod --yes"
  echo
  read -r -p "UrNammu rotated + redeployed? Type 'yes' to rotate collector: " confirm
  if [ "$confirm" != "yes" ]; then
  ...
  set_and_restart cursor-forward-bearer-token "$token"
}
...
case "$cmd" in
  ...
  rotate-cursor-forward) cmd_rotate_cursor_forward "$@" ;;
