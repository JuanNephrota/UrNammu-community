...
export async function loadEnforcementMode(): Promise<EnforcementMode> {
  ...
  // Fail closed: a DB error must not silently degrade to "off". Serve the
  // last known mode (even expired) if we have one; otherwise propagate so
  // the proxy refuses the request instead of skipping enforcement.
  ...
  try {
    ...
    if (modeCache) {
      console.error(
        "loadEnforcementMode: DB error, serving stale cached mode:",
        err
      );
...
export async function loadPoliciesForSystem(
  ...
  // Fail closed: never treat a DB error as "no policies" — that would let
  // requests through unenforced during outages. Stale cache beats fail-open;
  // with no cache, propagate so the caller refuses the request.
  let assignments;
  try {
    ...
    if (cached) {
      console.error(
        "loadPoliciesForSystem: DB error, serving stale cached policies:",
        err
      );
      return cached.value;
    }
