...
import { bearerTokenMatches, secretsMatch } from "@/lib/secret-compare";
...
async function requireNetskopeAuth(req: NextRequest): Promise<boolean> {
  ...
  if (secretsMatch(proxyKey, proxySecret)) return true;

  // Netskope log shipper uses Authorization: Bearer <token>
  const authHeader = req.headers.get("authorization");
  if (bearerTokenMatches(authHeader, proxySecret)) return true;
