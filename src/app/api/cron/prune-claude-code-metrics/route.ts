...
/**
 * Scheduled retention job for Claude Code OTel telemetry. Hit by Vercel Cron
 * via vercel.json. Guarded by `Authorization: Bearer $CRON_SECRET`.
 *
 * Deletes rows in ClaudeCodeMetric AND ClaudeCodeEvent older than the
 * configured retention window. The `timestamp` column is the OTel
 * wall-clock, which is what the UI filters on — so retention is measured in
 * client time, not server-received time. Indexes on `timestamp` keep the
 * deletes cheap.
 *
 * Config:
...
export async function GET(req: Request) {
  ...
  const [metrics, events] = await Promise.all([
    prisma.claudeCodeMetric.deleteMany({ where: { timestamp: { lt: cutoff } } }),
    prisma.claudeCodeEvent.deleteMany({ where: { timestamp: { lt: cutoff } } }),
  ]);
